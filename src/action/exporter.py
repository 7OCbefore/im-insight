...
class CsvExporter:
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        self.columns = [
            "Time", "Group", "Sender", "Intent", 
            "Item", "Price", "Specs", "RawContent"
        ]
    ...
    def save(self, signal: MarketSignal):
        ...
        try:
            with open(file_path, mode="a", encoding="utf-8-sig", newline="") as f:
                ...
                time_str = signal.timestamp.strftime("%Y-%m-%d %H:%M:%S") if isinstance(signal.timestamp, datetime) else str(signal.timestamp)
                
                row = {
                    "Time": time_str,
                    "Group": signal.group if signal.group else "Direct Message",
                    "Sender": signal.sender,
                    "Intent": signal.intent,
                    "Item": signal.item if signal.item else "",
                    "Price": signal.price if signal.price is not None else "",
                    "Specs": signal.specs if signal.specs else "",
            ...
            logger.error(f"PermissionError: Could not write to {file_path}. Is it open in Excel?")
